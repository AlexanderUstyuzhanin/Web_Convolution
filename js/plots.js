...
let samplePoints =[];   	// X-axis values for signals 1 & 2
let sliderSamplePoints = []; // slider dependent sample points
let samplePeriod = 1 / 128; // 128 samples per unit on the X-axis
let resultPoints =[];   	 // X-axis values for signals 3
...
function scaleResult(sigAr){
	let signal1 = document.getElementById("functionList1").value;
	let signal2 = document.getElementById("functionList2").value;
...
function generateResultPoints(pointAr, resAr){
    let N = resAr.length;
    let t_range = N * samplePeriod * multiplier; // get the required range of the X-axis
    let t_out = [];
    let t_start = -1*t_range/2, t_end = t_range/2; // define the starting and ending point for the X-axis
...
function start(brd,brd2){
	...
	s = brd.create('slider',[[1,1.5],[3,1.5],[-4,0,4]], {name:'t',snapWidth:0.05,  withLabel: true});
...
function plot1(brd){
	...
	cancelGraphicalView();
    let signal = document.getElementById("functionList1").value; // get the position value of the function plot

    let widthTextObj = document.getElementById("F1_width");
    let shiftTextObj = document.getElementById("F1_shift");
...
function plot2(brd){
	...
	cancelGraphicalView();
    let signal = document.getElementById("functionList2").value; // get the position value of the function plot

    let widthTextObj = document.getElementById("F2_width");
    let shiftTextObj = document.getElementById("F2_shift");
    widthTextObj.disabled = false;
...
function reDrawSignal2(){
	let arrayIndex;
		...
		graph2.updateDataArray = function(){
			...
			let signal = document.getElementById("functionList2").value;
			Q = shiftSignal2 - s.Value();
			if (convoCorr == 0){ // convolution
				if(signal == "6") {let xVal = s.Value() - (shiftSignal2);
											  //console.log(xVal);
...
function adjustSlider(){
	...
	if (currentCoordinateArray[2] < previousCoordinateArray[2]){ // zoom in
		let yCoord = 0.7826 * (currentCoordinateArray[1] - currentCoordinateArray[3]) + currentCoordinateArray[3];
